...
class BaseDataLoader:
    def __init__(self, endpoint=None, logger=None):
        ...
        if self._base_url is None:
            raise ValueError("Не вказано кінцеву точку API")
        if logger is None:
            logger = logging.getLogger('BASELDR')
        self._logger = logger
        self._logger.info("Створено екземпляр BaseDataLoader")

    def _make_request(self, method, resource, params=None):
        req_url = self._base_url + resource
        if params is not None:
            self._logger.debug(f"{method}: url={req_url}, params={params}")
            response = method(req_url, params=params)
        else:
            self._logger.debug(f"{method}: url={req_url}")
            response = method(req_url)
    ...
    def _get_req(self, resource, params=None):
        return self._make_request(requests.get, resource, params=params)
